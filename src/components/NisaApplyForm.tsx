...
interface NisaApplyFormProps {
  sessions: StudySession[];
  selectedSessionId?: string;
}

export default function NisaApplyForm({ sessions, selectedSessionId }: NisaApplyFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // イベント名にNISA初心者勉強会とわかる文言を付与
    const eventVal = String(data.event || '');
    let eventName = `NISA初心者勉強会 (${eventVal})`;
    
    if (eventVal === 'other') {
      eventName = 'NISA初心者勉強会 (日程選択: その他・個別調整希望)';
    } else {
      ...
      if (foundSession) {
        ...
        eventName = `NISA初心者勉強会 [${formatSessionDate(foundSession.date)} ${formatSessionTime(foundSession.date)}〜 ${isOffline ? '[対面]' : '[オンライン]'}]`;
      }
    }
    ...
    try {
      ...
      console.error('NISA submission error:', error);
      setStatus('error');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="apply-form">
      <input type="hidden" name="form-name" value="nisa-apply" />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          >
            <input
              ...
              value="other"
              required
              defaultChecked={!sessions || sessions.length === 0}
              style={{
                width: '18px',
                height: '18px',
                accentColor: 'var(--primary)',
      ...
      <div className="form-group">
        <label htmlFor="message" className="form-label">参加動機・事前のご質問</label>
        <textarea id="message" name="message" className="form-textarea" placeholder="NISAについて学びたいことやご希望日時があれば自由にご記入ください"></textarea>
      </div>
