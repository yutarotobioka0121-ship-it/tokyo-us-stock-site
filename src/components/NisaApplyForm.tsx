...
export default function NisaApplyForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // イベント名を「NISA初心者勉強会」として送信
    const payload = {
      name: data.name,
      email: data.email,
      event: `NISA初心者勉強会希望 (${data.studyStyle || '形式未指定'}) [ご希望日時: ${data.preferredDate || '指定なし'}]`,
      message: `【希望スタイル】${data.studyStyle || '未選択'}\n【希望日時】${data.preferredDate || '指定なし'}\n\n【事前の疑問・メッセージ】\n${data.message || '特になし'}`,
    };
    ...
    try {
      ...
      console.error('NISA apply submission error:', error);
      setStatus('error');
    }
  };
    ...
    return (
      <div className="success-message slide-up" style={{ textAlign: 'left', padding: '3rem 0', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
        ...
        <p style={{ color: 'var(--text-muted)', textAlign: 'left', lineHeight: '1.8' }}>
          ご入力いただいた希望日時を元に、運営より改めて日程確定・調整に関する案内メールをお送りいたしました。<br />
  ...
  return (
    <form onSubmit={handleSubmit} className="apply-form">
      ...
      <div className="form-group" style={{ marginBottom: '1.8rem' }}>
        <label className="form-label" style={{ fontWeight: '800', marginBottom: '0.8rem', display: 'block' }}>希望の受講スタイル</label>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.95rem', background: 'var(--bg-warm)', padding: '0.75rem 1.2rem', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.08)' }}>
            <input type="radio" name="studyStyle" value="対面参加（都内カフェ等）" defaultChecked />
            対面参加（都内カフェ・レンタルスペース）
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.95rem', background: 'var(--bg-warm)', padding: '0.75rem 1.2rem', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.08)' }}>
            <input type="radio" name="studyStyle" value="オンライン参加（Zoom）" />
            オンライン参加（Zoom）
          </label>
        </div>
      </div>
      ...
      <div className="form-group" style={{ marginBottom: '1.8rem' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.6rem' }}>
          例：8月2日(日) 14:00以降、平日19時以降 など（候補をいくつか挙げていただけますとスムーズです）
        </p>
        <input
          type="text"
          id="preferredDate"
          ...
          placeholder="例：第一希望 8/2(日) 14:00〜、第二希望 平日19:00以降"
      ...
      <div className="form-group" style={{ marginBottom: '2rem' }}>
        <label className="form-label" htmlFor="message" style={{ fontWeight: '800', marginBottom: '0.4rem', display: 'block' }}>
          事前の疑問・メッセージ <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 'normal' }}>（任意）</span>
        </label>
        <textarea
          id="message"
          name="message"
          rows={4}
          className="form-input"
          placeholder="つみたて投資枠と成長投資枠の使い分けについて知りたい等、気になる点があればご記入ください。"
